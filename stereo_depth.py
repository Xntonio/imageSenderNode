...
if __name__ == '__main__':
    ...
    while True:  # Loop until 'q' pressed or stream ends
        ...
        #leftFrame  = leftFrame[:-2,:].astype(np.float32)
        #rightFrame = rightFrame[:-2,:].astype(np.float32)

        leftFrame = 255*(leftFrame-min_value)/(max_value-min_value)
        rightFrame = 255*(rightFrame-min_value)/(max_value-min_value)

        #leftFrame = 255*(leftFrame-leftFrame.min())/(leftFrame.max()-leftFrame.min())
        #rightFrame = 255*(rightFrame-rightFrame.min())/(rightFrame.max()-rightFrame.min())
        #print("R",leftFrame.shape)
        #print("L",rightFrame.shape)

        height, width, channel = leftFrame.shape  # We will use the shape for remap

        # Undistortion and Rectification part!
        leftMapX, leftMapY = cv2.initUndistortRectifyMap(K1, D1, R1, P1, (width, height), cv2.CV_32FC1)
        left_rectified = cv2.remap(leftFrame.astype(np.uint8), leftMapX, leftMapY, cv2.INTER_LINEAR, cv2.BORDER_CONSTANT)
        rightMapX, rightMapY = cv2.initUndistortRectifyMap(K2, D2, R2, P2, (width, height), cv2.CV_32FC1)
        right_rectified = cv2.remap(rightFrame.astype(np.uint8), rightMapX, rightMapY, cv2.INTER_LINEAR, cv2.BORDER_CONSTANT)

        # We need grayscale for disparity map.
        gray_left = cv2.cvtColor(left_rectified.astype(np.uint8), cv2.COLOR_BGR2GRAY)
        gray_right = cv2.cvtColor(right_rectified.astype(np.uint8), cv2.COLOR_BGR2GRAY)

        disparity_image = depth_map(gray_left.astype(np.uint8), gray_right.astype(np.uint8))  # Get the disparity map

        # Show the images
        cv2.imshow('left(R)', leftFrame.astype(np.uint8))
        cv2.imshow('right(R)', rightFrame.astype(np.uint8))
        ...
        dim = (width, height)
        disparity_image = cv2.resize(disparity_image, dim, interpolation = cv2.INTER_AREA)

        #cv2.imshow('Disparity', disparity_image)
        #cv2.imshow('Disparity', disparity_image.astype(np.uint8))
        cv2.imshow('Disparity', disparity_image[90:410,310:580])
        cv2.waitKey(100)
